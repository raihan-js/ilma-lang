...
static void collect_run_stmts(ASTNode* program, ASTList* list) {
    if (program->type != NODE_PROGRAM && program->type != NODE_BLOCK) return;
    for (int i = 0; i < program->data.block.statements.count; i++) {
        ASTNode* stmt = program->data.block.statements.items[i];
        if (stmt->type == NODE_RUN_STMT) {
            ast_list_add(list, stmt);
        }
    }
}
        ...
        case NODE_CALL: {
            ...
            if (callee->type == NODE_IDENTIFIER) {
                ...
                if (is_blueprint_name(name)) {
                    ...
                    fprintf(cg->out, "ilma_print(");
                } else if (strcmp(name, "exit") == 0) {
                    fprintf(cg->out, "ilma_exit_program(");
                } else if (strcmp(name, "env") == 0) {
                    fprintf(cg->out, "ilma_env_get(");
                } else if (strcmp(name, "sleep") == 0) {
                    fprintf(cg->out, "ilma_sleep_ms(");
                } else if (strcmp(name, "timestamp") == 0) {
                    /* No-arg built-in — emit full call now, skip args loop */
                    fprintf(cg->out, "ilma_timestamp()");
                    free(safe);
                    break;
                } else if (strcmp(name, "args") == 0) {
                    /* No-arg built-in — emit full call now, skip args loop */
                    fprintf(cg->out, "ilma_args_get()");
                    free(safe);
                    break;
                } else {
...
static void gen_statement(CodeGen* cg, ASTNode* node) {
    /* NODE_TEST and NODE_ASSERT are only for ilma test mode; skip in normal compilation */
    if (node->type == NODE_TEST || node->type == NODE_ASSERT) return;

    /* NODE_RUN_STMT: spawn concurrent task (simplified: run synchronously) */
    if (node->type == NODE_RUN_STMT) {
        char* safe = sanitize_ident(node->data.run_stmt.task_name);
        emit(cg, "pthread_t _task_%s;\n", safe);
        /* For now, we call the function synchronously via a thread wrapper */
        /* The thread wrapper is emitted in the preamble */
        emit(cg, "pthread_create(&_task_%s, NULL, _ilma_thread_%s, NULL);\n", safe, safe);
        free(safe);
        return;
    }

    /* NODE_WAIT_STMT: wait for concurrent task */
    if (node->type == NODE_WAIT_STMT) {
        char* safe = sanitize_ident(node->data.wait_stmt.task_name);
        emit(cg, "pthread_join(_task_%s, NULL);\n", safe);
        free(safe);
        return;
    }
...
void codegen_generate(CodeGen* cg, ASTNode* program) {
    ...
    fprintf(cg->out, "#include <pthread.h>\n");
    ...
    /* Collect run statements and emit thread wrapper functions */
    ASTList run_stmts;
    ast_list_init(&run_stmts, 8);
    collect_run_stmts(program, &run_stmts);
    for (int i = 0; i < run_stmts.count; i++) {
        ASTNode* rs = run_stmts.items[i];
        char* safe = sanitize_ident(rs->data.run_stmt.task_name);
        fprintf(cg->out, "static void* _ilma_thread_%s(void* _arg) {\n", safe);
        fprintf(cg->out, "    (void)_arg;\n");
        fprintf(cg->out, "    ");
        gen_expression(cg, rs->data.run_stmt.call);
        fprintf(cg->out, ";\n");
        fprintf(cg->out, "    return NULL;\n");
        fprintf(cg->out, "}\n\n");
        free(safe);
    }
    free(run_stmts.items);
